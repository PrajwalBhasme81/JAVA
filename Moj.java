public class Moj
{
	public static void Videos()
	{
		String name = "Moj:Viral Short Videos";
		System.out.println("The Name of the App is :"+name);
	}
	
	public static void Videos(String Type, int Size)
	{
		System.out.println("The App Type is:"+Type+"  "+"The App size is:"+Size);
	}
	
	public static String Videos(float rating)
	{
		String downloads = "10Cr+";
		System.out.println("The Total Downloads is :"+downloads+"  "+"The Rating is:"+rating);
		return "MethodEnd";
	}
	
	public static String Videos(String features[])
	{
		for(int index = 0; index < features.length; index++)
		System.out.println("The Some Features are:"+features[index]);
		return "Method End";
	}
}
